...
=head2 CMD
    ...
    Example  : my @out = Bio::RNASeq::Utils::CMD('ls',{'verbose'=>1,'exit_on_error'=>0});
               my $fh_in  = Bio::RNASeq::Utils::CMD('ls',{rpipe=>1}); while (my $line=<$fh>) { print $line; } close($fh_in);
               my $fh_out = Bio::RNASeq::Utils::CMD('gzip -c > out.gz',{wpipe=>1}); print $fh_out "test\n"; close($fh_out);
    Options  :
...
sub CMD
{
    ...
    if ( $exit_status )
    {
        ...
        my $bt = Bio::RNASeq::Utils::backtrace();
        my $msg = join('',@$bt) . "\n" . join('',@msg) . "\n";
        log_msg($$options{'logfile'},$msg) unless !exists($$options{'logfile'});

        Bio::RNASeq::Utils::error(@msg) unless !$$options{'exit_on_error'};
        print STDERR $msg;
    }
...
sub relative_symlink
{
    ...
    if ( !scalar @cur_els || !scalar @new_els ) { Bio::RNASeq::Utils::error("FIXME: $cur, $new\n") }
    ...
    chdir($dir) or Bio::RNASeq::Utils::error("chdir $dir: $!");
    ...
    symlink($path,$new_els[0]) or Bio::RNASeq::Utils::error("symlink $path $new_els[0]: $!");

    chdir($cwd) or Bio::RNASeq::Utils::error("chdir $cwd: $!");
    return;
}
...
{
    my ($path,$suffix) = @_;
    if ( !($path =~ m{/?([^/]+)/?$}) ) { Bio::RNASeq::Utils::error("FIXME: could not parse \"$path\".\n") }
    my $dir  = $` ? $` : '';
    my $base = $1;
    my $suff = '';
    if ( $suffix )
...
{
    my ($afile,$bfile) = @_;
    my (@astat) = stat($afile) or Bio::RNASeq::Utils::error("stat $afile: $!");
    my (@bstat) = stat($bfile) or Bio::RNASeq::Utils::error("stat $bfile: $!");
...
sub fai_chromosome_lengths
{
    ...
    open(my $fh, '<', $fai_file) or Bio::RNASeq::Utils::error("$fai_file: $!");
    while (my $line=<$fh>)
    {
        if ( !($line=~/^(\S+)\s+(\d+)\s+/) ) { Bio::RNASeq::Utils::error("Unexpected format of $fai_file:\n$line\n"); }
        $$out{$1} = $2;
    }
    close $fh;
...
sub mysql_exec
{
    ...
    if ( !$sth ) { Bio::RNASeq::Utils::error("$query:\n", $!) }

    $sth->execute or Bio::RNASeq::Utils::error("$query:\n", $!);
    $sth->finish;
    return;
}
