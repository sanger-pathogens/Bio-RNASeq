...
sub _simple_positions {
    my ($self) = @_;
    my @bases;
    my $position = $self->_read_position;
    my $cigar    = $self->_read_details->{cigar};

    while ( $cigar !~ /^$/ ) {
        if ( $cigar =~ /^([0-9]+[MIDN])/ ) {
            my $cigar_part = $1;
            if ( $cigar_part =~ /(\d+)M/ ) {
                my $position_offset = $1;
                for ( my $i = $position ; $i < $position + $position_offset ; $i += $self->_base_position_interval ) {
                    push( @bases, $i );
                }
...
sub _build__base_positions {
    ...
    my ($self) = @_;
    if ( $self->_read_details->{cigar}  =~ /^([0-9]+[MIDN])+$/ ) {
        return $self->_simple_positions();
    }
    else {
