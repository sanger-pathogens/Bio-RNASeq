...
sub plot_stats
{
    ...
    if ( !exists($$stats{'data'}) ) { Bio::RNASeq::VertRes::Utils::Utils::error("FIXME\n") }
    if ( !exists($$stats{'outfile'}) ) { Bio::RNASeq::VertRes::Utils::Utils::error("FIXME\n") }

    if ( !($$stats{'outfile'} =~ /([^.]+)$/ ) ) # Match the suffix
    { 
        Bio::RNASeq::VertRes::Utils::Utils::error("Could not determine the filetype of \"$$stats{'outfile'}\".\n") 
    }
    my $file_type = $1;
    ...
    if ( $barplot && @{$$stats{'data'}} > 1 ) { Bio::RNASeq::VertRes::Utils::Utils::error("TODO: multiple barplots in one graph.\n"); }

    open(my $fh, '>', "$$stats{'outfile'}.R") or Bio::RNASeq::VertRes::Utils::Utils::error("$$stats{'outfile'}.R: $!");
    ...
    {
        if ( !$vals ) { Bio::RNASeq::VertRes::Utils::Utils::error("Given ampty data set for $$stats{'outfile'}\n") }
        if ( !scalar @{$$vals{'xvals'}} ) { Bio::RNASeq::VertRes::Utils::Utils::error("The data set is empty for $$stats{'outfile'}\n") }
...
yrange <- range($yrange)
    ...
    Bio::RNASeq::VertRes::Utils::Utils::CMD("cat $$stats{'outfile'}.R | $R_CMD --slave --vanilla");
    return;
}
...
sub plot_histograms_distributions {
    ...
    if ($hash_in->{outfile} =~ /([^.]+)$/ ) { 
        ...
        Bio::RNASeq::VertRes::Utils::Utils::error("Could not determine the filetype of $hash_in->{outfile}\n");
    }
    ...
    foreach my $i (0 .. (scalar @{$hash_in->{xdata}} - 1)) {
        ...
        my %stats = Bio::RNASeq::VertRes::Bio::RNASeq::VertRes::Utils::Utils::Math->new()->histogram_stats($hash_in->{ydata}->[$i]);
    ...
    # write and run the R script
    open my $fh, '>', "$hash_in->{outfile}.R" or Bio::RNASeq::VertRes::Utils::Utils::error("$hash_in->{outfile}.R: $!");
    print $fh <<R_SCRIPT;
...
R_SCRIPT
    ...
    close $fh;
    Bio::RNASeq::VertRes::Utils::Utils::CMD("cat $hash_in->{outfile}.R | $R_CMD --slave --vanilla");
}
...
sub create_gc_depth_graph
{
    ...
    open(my $fh, '>', "$png_file.R") or Bio::RNASeq::VertRes::Utils::Utils::error("$png_file.R: $!");
    print $fh qq[
source('$gcdepth_R')
...
gcdepth(depdat, sname = '$png_file', depmax = NULL, hc = TRUE, plotdev=bitmap, nbins = 30, binned = TRUE)
    ...
    close $fh;
    Bio::RNASeq::VertRes::Utils::Utils::CMD("cat $png_file.R | $R_CMD --slave --vanilla");
