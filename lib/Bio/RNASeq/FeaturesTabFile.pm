...
sub _build__output_file_handles
{
  ...
  {
	  $output_file_handles{$sequence_name} = new IO::Compress::Gzip $self->output_filename.".$sequence_name.tab.gz"  or Bio::RNASeq::Exceptions::FailedToOpenFeaturesTabFileForWriting->throw( error => "Cant open ".$self->output_filename." for writing");
	}
